...
model{
	...
	for(i in 1:n){
		...
		# m <- exp(a1+b1*(MAT[i]-MAT.bar)/MAT.sdev)/(1+exp(a1+b1*(MAT[i]-MAT.bar)/MAT.sdev)
		y[i]~dnorm(int+exp(a1+b1*(MAT[i]-MAT.bar)/MAT.sdev)/(1+exp(a1+b1*(MAT[i]-MAT.bar)/MAT.sdev))+a2*(MAP[i]-MAP.bar)/MAP.sdev,sigma)
	}
