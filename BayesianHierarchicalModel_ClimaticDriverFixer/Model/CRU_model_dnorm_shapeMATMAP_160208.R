...
model{
	...
	for(i in 1:n){
		...
		# map_m <- 1/(1+exp(-(a2+b2*(MAP[i]-MAP.bar)/MAP.sdev+c2*((MAP[i]-MAP.bar)/MAP.sdev)^2)))# quadratic exponential
		y[i]~dnorm(int+exp(a1+b1*(MAT[i]-MAT.bar)/MAT.sdev)/(1+exp(a1+b1*(MAT[i]-MAT.bar)/MAT.sdev))+1/(1+exp(-(a2+b2*(MAP[i]-MAP.bar)/MAP.sdev+c2*((MAP[i]-MAP.bar)/MAP.sdev)^2))),sigma)
	}
	...
	y_pred_MAP[k] <- int+1/(1+exp(-(a2+b2*(MAP_x[k]-MAP.bar)/MAP.sdev+c2*((MAP_x[k]-MAP.bar)/MAP.sdev)^2)))
	}
