### Climatic Driver of N-fixers
### Author: WL
### Bayesian hierarchical models

##################################
################################## # First: Normal error structure
##################################
...
inits=list(list(mu1=1,tau1=1,a2=1.9,int=0.01,sigma=0.1),list(mu1=1,tau1=1,a2=1.9,int=0.01,sigma=0.1),list(mu1=1,tau1=1,a2=1.9,int=0.01,sigma=0.1))
inits=list(list(mu1=1,tau1=1,mu2=1.1,tau2=1.1,int=0.01,sigma=0.1),list(mu1=1,tau1=1,mu2=1.1,tau2=1.1,int=0.01,sigma=0.1),list(mu1=1,tau1=1,mu2=1.1,tau2=1.1,int=0.01,sigma=0.1))
...
#setwd("")
dnorm_model_jags <- jags.model("normMAT_linearMAP.R",data=data,inits,n.chains=length(inits),n.adapt=n.adapt)
dnorm_model_jags <- jags.model("normMAP.R",data=data,inits,n.chains=length(inits),n.adapt=n.adapt)
update(dnorm_model_jags,n.iter=n.update)
dnorm_model_coda=coda.samples(dnorm_model_jags,variable.names=c("mu1","tau1","mu1","tau2","int","sigma"),n.iter=n.iter,thin=1)
...
#setwd("")
pdf("CRU_160218_normMAT_normMAP.pdf",width=9,height=4.5)
